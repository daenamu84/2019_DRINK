	...
	$( function(){
		...
		$("#setp03Insert").click(function(){
			...
			for(var i=0; i<prpsdId.length; i++){
				var idx = prpsdId[i];
				var deliveryCntObj = $("input[name='real_delivery_cnt'][data-prpsdid='"+idx.value+"']");
				var planCntObj = $("input[name='planCnt'][data-prpsdid='"+idx.value+"']");
				var real_deliveryCntnObj = $("input[name='real_delivery_cntn'][data-prpsdid='"+idx.value+"']");
				var tCnt = 0;
				var sumCnt = 0;
			  ...
			  <tbody>
		  		<c:forEach items="${listStep03}" var="i" varStatus="status">
				  	<tr>
				  		...
				  		<td class="border">
				  			<c:forEach items="${i.dateList}" var="d">
				  				<input type="number" min="0" id="planCnt" class="form-control" name="planCnt"  data-prpsdid="${i.PRPSD_ID}" style="margin-bottom:3px;" value="${d.PLAN_DELIVERY_CNT}" readonly>
				  			</c:forEach>
				  		</td>
				  		<td class="border">
				  			<c:forEach items="${i.dateList}" var="d">
				  				<input type="number" min="0" id="real_delivery_cnt" class="form-control" name="real_delivery_cnt" data-prpsdid="${i.PRPSD_ID}" data-deliverycnt="${i.DELIVERY_CNT}" style="margin-bottom:3px;" value="${d.REAL_DELIVERY_CNT}" >
				  			</c:forEach>
				  		</td>
				  		<td class="border">
				  			<c:forEach items="${i.dateList}" var="d">
				  				<input type="text" min="0" id="real_delivery_cntn" class="form-control" name="real_delivery_cntn"  data-prpsdid="${i.PRPSD_ID}"  style="margin-bottom:3px;" value="${d.REAL_DELIVERY_CNTN}" >
				  			</c:forEach>
				  		</td>
				  	</tr>
				</c:forEach>
